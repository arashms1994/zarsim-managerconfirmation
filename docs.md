...
1. کاربر روی دکمه تأیید کلیک می‌کند → حداقل ۵ ثانیه لودینگ (اسپینر و «در حال پردازش...») نمایش داده می‌شود.
...
1. کاربر روی دکمه رد کلیک می‌کند → حداقل ۵ ثانیه لودینگ نمایش داده می‌شود.
...
- **صفحه اصلی:** `ChangeHistoryTable`: جدول تاریخچه تغییرات، فیلتر بر اساس کاربر، جست‌وجو، صفحه‌بندی، کلیک ردیف برای مودال. تعریف ستون‌ها از `Colomns.tsx` ایمپورت می‌شود.
- **تعریف ستون‌ها:** `Colomns.tsx`: تعریف ستون‌های جدول (شماره ردیف، شرح محصول، تاریخ درخواست، عملیات) با استفاده از jalali-moment برای فرمت تاریخ شمسی.
- **ستون عملیات:** `ActionsCell`: دکمه تأیید/رد، لودینگ حداقل ۵ ثانیه (با اولویت بالاتر از وضعیت، تا حتی پس از آپدیت داده هم نمایش داده شود)، به‌روزرسانی وضعیت بعد از پاسخ سرور.
- **مودال:** `Modal`: نمایش جزئیات پیش‌فاکتور و اصلاحات با داده از `useDetailCustomerFactor(rowData?.Title)`. از دو کامپوننت استفاده می‌کند:
  - **PreInvoiceRowDetail**: نمایش جزئیات در پیش‌فاکتور (بخش قرمز)
  - **EditedPreInvoiceRowDetail**: نمایش جزئیات اصلاحات (بخش سبز)
